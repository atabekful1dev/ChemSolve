...
if st.button("Тіктөртбұрыштар әдісіне есеп шығару"):
    with st.form("calculation_form"):
        ...
        m1_str = st.text_input("m1 (масса первого компонента)")
        w1_str = st.text_input("w1 (доля первого компонента)")
        m2_str = st.text_input("m2 (масса второго компонента)")
        w2_str = st.text_input("w2 (доля второго компонента)")
        mw_str = st.text_input("mw (средневзвешенная доля)")
